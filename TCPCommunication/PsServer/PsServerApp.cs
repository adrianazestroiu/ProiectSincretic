...
namespace PsServer
{
    public class PsServerApp
    {
        ...
        public Task BroadCastMessage(byte data)
        {
            //take a copy of the workers, since the list might change while iterating through it
            var workersCopy = _workers.Values.ToArray();
            var tasks = new List<Task>();
            foreach(var worker in workersCopy)
            {
                tasks.Add(worker.Client.Send(data));
            }
